...
export function UserTableRow({ user, onEdit, onDelete }: UserTableRowProps) {
  return (
    <tr className="border-t border-slate-200 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/50">
      ...
      <td className="px-4 py-3 text-sm">
        <span className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${getStatusBadgeClass(user.isActive)}`}>
      ...
      <td className="px-4 py-3 text-sm">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
          >
            Delete
          </button>
        </div>
