...
export function ManagementEventEditPage({ eventId }: { eventId: string }) {
  const {
    formState,
    formErrors,
    formSummaryErrors,
    ...
    confirmLoadDraft,
    cancelLoadDraft,
  } = useManagementEventEdit(eventId);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Edit Event</h2>
      ...
      <WarningModal
        isOpen={Boolean(draftToLoad)}
        onClose={cancelLoadDraft}
        onConfirm={confirmLoadDraft}
        title="Saved Draft Found"
        message="A saved draft exists for this event. Do you want to load it?"
